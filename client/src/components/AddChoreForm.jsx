...
const AddChoreForm = (props) => {
    const {setChoreList, choreList, initialChoreName, initialChoreDescription, initialChoreLocation, initialChorePostedBy, initialChoreResponsibility, buttonStyle, editStyle} = props;
    const [choreName, setChoreName] = useState(initialChoreName);
    const [choreDescription, setChoreDescription] = useState(initialChoreDescription);
    ...
    const onSubmitHandler = (e) => {
        e.preventDefault();
        axios.post('http://localhost:8000/api/chores', ({choreName, choreDescription, choreLocation, chorePostedBy, choreResponsibility}))
            .then(res => {
                    console.log("THIS IS RES:", res);
                console.log(res.data)
                console.log("THIS IS res.data._id: ", res.data._id)
                setChoreList([...choreList, res.data])
                setChoreID(res.data._id)
                navigate("/dashboard")
                // navigate("/chores/" + res.data._id)
                })
            .catch((err)=> {
